...
import { useLogOutMutation } from "@/redux/features/home/fcmApi";
...
export default function Profile() {
  ...
  const [createSupportConversation, { isLoading }] =
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: Lock,
      onPress: () =>
        router.push("/(settings)/changePasswordFromSettings" as any),
    },
    {
      id: "3",
      title: "Support",
    ...
    },
    {
      ...
      icon: ScrollText,
      onPress: () => router.push("/(settings)/termsCondition" as any),
    },
  ];
  ...
  const handleLogout = async () => {
    try {
      setLoggingOut(true);
      if (fcmToken) {
        await logOut({ fcmToken } as any).unwrap();
      }
    } catch (error) {
      console.log("❌ Logout API failed: ", error);
      // proceed with local logout even if API fails, so user isn't stuck
    } finally {
